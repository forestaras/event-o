...
<link rel='stylesheet' href='https://cdnjs.cloudflare.com/ajax/libs/bootstrap-year-calendar/1.1.1/css/bootstrap-year-calendar.css'>
  ...
  <div class="modal-dialog">
    <div class="modal-content">
      <div class="modal-header">
         ...
         <p><h3 id="title">Event title</h3></p>
        </h4>
      ...
      clickDay: function(e) {
        if (e.events.length > 0) {
          ...
          for (var i in e.events) {
            ...
            $('#url').html('<a class="btn btn-primary" href="/event/'+ e.events[i].id +'" role="button">На сторінку змагань</a>');
            ...
            $('#p').html('<div class="event-tooltip-content">' + '<div class="event-name" style="color:' + e.events[i].color + '"> <h3>'+ e.events[i].name + '</h3></div>' + '<div class="event-location">' + e.events[i].location + '</div>' + '</div>');
      ...
      @foreach($event as $even)
       {
        ...
        obl: '{{$even->obl->title}}',
        org: '{{$even->org}}',
        startDate: new Date({{$even->ys}}, {{$even->ms}}, {{$even->ds}}),
        start:'{{$even->ds}}.{{$even->ms}}.{{$even->ys}}',
        endDate: new Date({{$even->yf}},{{$even->mf}},{{$even->df}}),
